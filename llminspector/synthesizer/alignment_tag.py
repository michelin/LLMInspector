"""Tag-replacement helper for the alignment engine.

Ports ``alignment_replace_function.tag_replace`` and the
``KeywordNotFoundException`` verbatim. Used by the legacy alignment engine's
tag-augmentation stage; kept as a small reusable primitive outside the engine.
"""
...
class tag_replace:
    ...
    def replace_tag(self, sentences, keyword, keyword_dict):
        ...
        for sentence in sentences:
            if key_tag_lower in sentence.lower() and key_tag_lower in map(
                str.lower, keyword_dict.keys()
            ):
                lst = [sentence]
